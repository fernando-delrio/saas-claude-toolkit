...
## Cómo reportar
Hallazgos ordenados por gravedad, **etiquetando [SEGURIDAD] o [ESCALADO]**, con: `archivo:línea`, el riesgo concreto (escenario de ataque o de carga) y el arreglo **con el idiom del stack detectado**. Antes de dar un arreglo por completo, traza la excepción que propone hasta la respuesta HTTP real (¿la capa superior —router/controller— la captura y la convierte en un error controlado, o se propagaría como un 500 sin manejar?). Si la capa superior no la captura, ese hueco es parte del mismo hallazgo, no algo aparte. No inventes problemas donde no los hay. Cierra distinguiendo lo **bloqueante para producción** de la **mejora futura** (YAGNI: no exijas Redis a 1 worker sin tráfico).
